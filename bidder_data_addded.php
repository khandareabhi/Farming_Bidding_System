...
$role = "Bidder";           // Set role as 'Bidder'

// Validate pincode (must be exactly 6 digits)
if (!preg_match("/^\d{6}$/", $c)) {
    die("<script>alert('Pincode must be exactly 6 digits.'); window.location.href='a.php';</script>");
}

if (!preg_match("/^[A-Za-z\s]{3,}$/", $a)) {
    die("<script>alert('Full Name must be at least 3 characters and contain only letters.'); window.location.href='a.php';</script>");
}

// Validate city (only letters and spaces, min 2 characters)
if (!preg_match("/^[A-Za-z\s]{2,}$/", $b)) {
    die("<script>alert('City must be at least 2 characters and contain only letters.'); window.location.href='a.php';</script>");
}

// Validate email format
if (!filter_var($f, FILTER_VALIDATE_EMAIL)) {
    die("<script>alert('Invalid email format. Please enter a valid email address.'); window.location.href='a.php';</script>");
}

// Check if passwords match
if ($g !== $aa) {
    die("<script>alert('Password does not match.'); window.location.href='a.php';</script>");
}
...
if (!in_array($imageFileType, $allowedFormats)) {
    die("<script>alert('Only JPG, JPEG, PNG & GIF files are allowed.'); window.location.href='a.php';</script>");
}
...
if ($stmt === false) {
    die("Prepare failed: " . phpspecialchars($conn->error));
}
